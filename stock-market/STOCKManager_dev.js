...
export async function main(ns) {
  ...
  while (true) {
    ...
    if (pre4s && !mock && await tryGet4SApi(ns, playerStats, bitnodeMults, corpus)) continue;
    ...
    try {
      const hackedServers = ns.read("/data/hacked-servers.txt").split("\n").filter(s => s);
    } catch (err) {
      ns.print("WARNING: Failed to parse /logs/batchPlans.txt for hacked servers.");
    }
    ...
    if (liquidity > fracB) {
      ...
      for (const stk of sortedStocks) {
        const server = symbolToServer(stk.sym);
        if (hackedServers.includes(server)) {
          if (!disableShorts && stk.bearish()) {
            ns.print(`Preferring to short ${stk.sym} because ${server} is being hacked`);
            // Continue to short logic
          } else {
            ns.print(`Skipping ${stk.sym} because ${server} is being hacked`);
            continue;
          }
        }
        ...
        if (isNaN(corpus) || isNaN(stk.positionValue())) {
          ns.print(`ERROR: Invalid corpus or position value for ${stk.sym}`);
          continue;
        }
        ...
        if (isNaN(estEndOfCycleValue)) {
          ns.print(`ERROR: Invalid estimated cycle value for ${stk.sym}`);
          continue;
        }
        if (estEndOfCycleValue <= 2 * commission) {
          ns.print(`Despite attractive ER of ${formatMoney(stk.absReturn())}, ${stk.sym} was not bought. Budget: ${format_compact_currency(budget)} can only buy ${numShares} shares @ ${formatMoney(purchasePrice)}. ` +
            ...
            `remaining ${format_compact_currency(ticksBeforeCycleEnd)} ticks would only generate ${format_compact_currency(estEndOfCycleValue)}, which is less than 2x commission (${format_compact_currency(2 * commission)})`);
        } else {
...
function initAllStocks(ns) {
  ...
  return stockSymbols.map(s => ({
    ...
    expectedReturn: function () {
      if (isNaN(this.vol) || isNaN(this.prob)) return 0;
...
function doStatusUpdate(ns, stocks, myStocks, hudElement = null) {
  ...
  ns.print(`Long ${myStocks.filter(s => s.sharesLong > 0).length}, Short ${myStocks.filter(s => s.sharesShort > 0).length} of ${stocks.length} stocks ` +
    ...
    `(Cost: ${format_compact_currency(est_holdings_cost)}) Net: ${format_compact_currency(totalProfit + liquidation_value - est_holdings_cost)})`);

  if (hudElement) hudElement.innerText = format_compact_currency(liquidation_value);
}

async function tryGet4SApi(ns, playerStats, bitnodeMults, corpus) {
  const baseCost = 100000e9; // 5B for 4S Market Data, 25B for 4S Market Data TIX API
