...
export async function main(ns) {
  ...
  if (flags.console) {
    const bestPlans = servers.mostProfitableServers(flags);
    const exportData = bestPlans.map(plan => ({
      hostname: plan.server.hostname,
      batch: plan.batch,
      delays: plan.batch.getDelays?.(), // if implemented
      moneyPerSec: plan.moneyPerSec,
      peakRam: plan.peakRam,
      numBatches: plan.numBatchesAtOnce,
      condition: plan.condition
    }));
    ns.write("/logs/batchPlans.txt", JSON.stringify(exportData, null, 2), "w");
  }
