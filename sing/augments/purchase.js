...
import {
  ...
  isUsefulPrograms,
  isZeroCost
} from '/cortex/augmentations/utils.js';
import { formatMoney, getFactions, getScripts } from '/extendedUtils.js';
...
export async function main(ns) {
  ...
  // Fallback: if no criteria were passed, assume all
  if (criteria.length === 0) {
    ns.tprint("No criteria passed — defaulting to --all.");
    criteria.push(
      isUsefulHacking,
      isUsefulCombat,
      isUsefulCrime,
  ...
  if (ns.stock.hasTIXAPIAccess()) { // Check if player has TIX API
    ...
    if (stocks && await ns.prompt(`Do you want to sell all shares?`)) ns.exec('/stock-manager/CASHout.js');
  }
...
// Purchase Stanek's Gift only if BitNode 13 is active or Source File 13 is owned
const hasStanekAccess = ns.getPlayer().bitNodeN === 13 ||
    ns.singularity.getOwnedSourceFiles().some(s => s.n === 13);

if (hasStanekAccess) {
    await purchaseAugmentations(ns, [isZeroCost]);
} else {
    ns.tprint("Skipping Stanek's Gift — BitNode 13 not unlocked.");
}

  // Ask to install augmentations
  if (options.install && await ns.prompt('Install augmentations?')) ns.singularity.installAugmentations('sing/autostart.js');
}
...
async function purchaseAugmentations(ns, criteria) {
  ...
  for (const faction of getFactions()) {
    for (const aug of ns.singularity.getAugmentationsFromFaction(faction)) {
      if (isUseful(ns, criteria, aug) && isPurchasable(ns, faction, aug, augmentations)) {
        ...
        //ns.tprint(`${aug} stats: ${JSON.stringify(stats)}`);
  ...
  if (augmentations.length === 0) {
    ns.tprint("No augmentations matched criteria.");
    return false;
  }
