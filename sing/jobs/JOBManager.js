...
export async function main(ns) {
    ...
    const fieldFlag = ns.args.find(arg => typeof arg === "string" && arg.startsWith("--field "));
    const customField = fieldFlag ? fieldFlag.split("=")[1] : null;

    if (!company) {
        ns.tprint("Usage: run JOBManager.js [company] [--field=Software|IT|Research]");
        return;
    }
    ...
    if (ns.getPlayer().city !== city) {
        const cost = ns.getTravelCost(city);
