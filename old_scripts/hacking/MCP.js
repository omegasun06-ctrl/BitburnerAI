...
function get_action(ns, host) {
  ...
  return actions[0].filename.replace("/newhome/daemons/", "").replace(".js", "")
}
...
function update_local(ns, TARGET) {
  ...
  for (let i = 0; i < local_servers.length; ++i) {
    ...
    ns.killall(serv, 1);
    ns.scp("/newhome/daemons/hack.js", serv);
    ns.scp("/newhome/daemons/weaken.js", serv);
    ns.scp("/newhome/daemons/grow.js", serv);

    const mem = ns.getServerMaxRam(serv);
    const scriptmem = ns.getScriptRam("/newhome/daemons/hack.js", "home");
    const threads = Math.floor(mem / scriptmem);

    ns.exec("/newhome/daemons/weaken.js", serv, Math.floor(threads / 3), TARGET);
    ns.exec("/newhome/daemons/grow.js", serv, Math.floor(threads / 3), TARGET);
    ns.exec("/newhome/daemons/hack.js", serv, Math.floor(threads / 3.5), TARGET);
